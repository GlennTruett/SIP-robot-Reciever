...
int radioNumber = 5;
bool role = false;
String payload = "";  //String length of 20 characters
...
void setup() {
  ...
  //}
  radioSetup("switch", radio, 0, 4, address);
}

void loop() {
  // put your main code here, to run repeatedly
  payload = radioReceive(vfNum, radio);
  if(payload != payloadState){
  vfNum = decryptVNum(payload);
  vry = decryptUpDown(payload);
  vrx = decryptLeftRight(payload);
...
String radioReceive(int vfNum, RF24 radio) {  // Function for receiving a transmission
  radio.startListening();
  uint8_t pipe;
  String payload = "";
